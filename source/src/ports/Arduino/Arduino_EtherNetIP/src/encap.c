...
/*encap.c was modified for Opta/Mbed socket stability and transaction reporting.
 *It adds transaction status reporting, checks socket-timer allocation, 
 *changes UnregisterSession socket ownership, and makes delayed UDP transmission safer.
 */
#include <string.h>
#include <stdlib.h>
#include <stdbool.h>
...
extern void OpenerRecordTransaction(const char *transport, unsigned int command, unsigned long request_bytes, long response_bytes, bool success);
                                       ...
                                       ENIPMessage *const outgoing_message)
{
  ...
  {
    const bool transaction_success =
        (return_value != kEipStatusError) ||
        (encapsulation_data.command_code == kEncapsulationCommandUnregisterSession);
    const long transaction_response =
        (return_value > 0) ? (long)outgoing_message->used_message_length : 0L;
    OpenerRecordTransaction("TCP", (unsigned int)encapsulation_data.command_code,
                            (unsigned long)length, transaction_response,
                            transaction_success);
  }
                                       ...
                                       bool unicast, ENIPMessage *const outgoing_message)
{
  ...
  }
  {
    ...
    const long transaction_response =
        (return_value > 0) ? (long)outgoing_message->used_message_length : 0L;
    OpenerRecordTransaction("UDP", (unsigned int)encapsulation_data.command_code,
                            (unsigned long)buffer_length, transaction_response,
                            transaction_success);
  }
...
{
  return sizeof(CipUint) + sizeof(CipInt) + sizeof(CipUint) + sizeof(CipUdint) + 8 * sizeof(CipUsint) + sizeof(CipUint) + sizeof(CipUint) + sizeof(CipUint) + 2 * sizeof(CipUsint) + sizeof(CipWord) + sizeof(CipUdint) + sizeof(CipUsint) + g_identity.product_name.length + sizeof(CipUsint);
}
...
void HandleReceivedRegisterSessionCommand(int socket, const EncapsulationData *const receive_data, ENIPMessage *const outgoing_message)
{
  ...
  if ((0 < protocol_version) && (protocol_version <= kSupportedProtocolVersion) && (0 == option_flag))
  { /*Option field should be zero*/
    ...
    if (kSessionStatusInvalid != session_index)
    {
      ...
      { /* successful session registered */
        ...
        {
          OPENER_TRACE_ERR("encap.c: no free socket timer for socket %d\n", socket);
          encapsulation_protocol_status = kEncapsulationProtocolInsufficientMemory;
          session_index = kSessionStatusInvalid;
        }
        else
...
EipStatus HandleReceivedUnregisterSessionCommand(const EncapsulationData *const receive_data, ENIPMessage *const outgoing_message)
{
                                             ...
                                             OPENER_NUMBER_OF_SUPPORTED_SESSIONS))
  {
    ...
    {
      /* The current TCP socket is being processed by generic_networkhandler.c.
       * Do not close/delete it from inside the encapsulation handler. Return an
       * error so the network handler performs the actual socket close after this
       * function returns. This avoids invalidating the active Mbed socket while
...
void ManageEncapsulationMessages(const MilliSeconds elapsed_time)
{
  for (size_t i = 0; i < ENCAP_NUMBER_OF_SUPPORTED_DELAYED_ENCAP_MESSAGES; i++)
  {
    if (kEipInvalidSocket != g_delayed_encapsulation_messages[i].socket)
    {
      ...
      {
        /* If delay is reached or passed, send the UDP message. Mark the entry free
         * before the socket call so the delayed-message slot cannot be reused
         * recursively while the network stack is active. */
        const int delayed_socket = g_delayed_encapsulation_messages[i].socket;
        const size_t delayed_length = g_delayed_encapsulation_messages[i].outgoing_message.used_message_length;
        g_delayed_encapsulation_messages[i].socket = kEipInvalidSocket;
        OPENER_TRACE_INFO("encap.c: delayed UDP send begin fd %d length %u\n", delayed_socket, (unsigned)delayed_length);
        const int delayed_sent = sendto(delayed_socket, (char *)g_delayed_encapsulation_messages[i].outgoing_message.message_buffer,
                                        delayed_length, 0, (struct sockaddr *)&(g_delayed_encapsulation_messages[i].receiver),
                                        sizeof(struct sockaddr));
        OPENER_TRACE_INFO("encap.c: delayed UDP send end fd %d result %d\n", delayed_socket, delayed_sent);
      }
    }
  }
}
...
void CloseEncapsulationSessionBySockAddr(const CipConnectionObject *const connection_object)
{
  for (size_t i = 0; i < OPENER_NUMBER_OF_SUPPORTED_SESSIONS; ++i)
  {
    if (kEipInvalidSocket != g_registered_sessions[i])
    {
      ...
      if (getpeername(g_registered_sessions[i], (struct sockaddr *)&encapsulation_session_addr, &addrlength) < 0)
      { /* got error */
...
void CloseClass3ConnectionBasedOnSession(CipSessionHandle encapsulation_session_handle)
{
  ...
  while (NULL != node)
  {
    ...
    if (kConnectionObjectTransportClassTriggerTransportClass3 == ConnectionObjectGetTransportClassTriggerTransportClass(connection_object) && connection_object->associated_encapsulation_session == encapsulation_session_handle)
    {
