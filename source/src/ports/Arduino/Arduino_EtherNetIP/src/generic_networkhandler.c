...
/* generic_networkhandler.c was adapted from the POSIX/lwIP networking model to the Arduino Opta/Mbed socket layer.
 * It introduces the Opta fd/time types and opener_select(),
 * adds socket-timer error handling,
 * moves large network buffers to static storage, and adds startup diagnostics.
 */
...
/* global vars */
opener_fd_set_t master_socket;
opener_fd_set_t read_socket;

int highest_socket_handle;
int g_current_active_tcp_socket;

opener_timeval_t g_time_value;
...
EipStatus NetworkHandlerInitialize(void)
{
  ...
  OPENER_TRACE_INFO("INIT: before FD_SET TCP\n");
  FD_SET(g_network_status.tcp_listener, &master_socket);
  OPENER_TRACE_INFO("INIT: after FD_SET TCP bits=0x%08lx%08lx\n",
                    (unsigned long)(master_socket.bits >> 32),
                    (unsigned long)(master_socket.bits & 0xFFFFFFFFUL));

  OPENER_TRACE_INFO("INIT: before FD_SET UDP unicast\n");
  FD_SET(g_network_status.udp_unicast_listener, &master_socket);
  OPENER_TRACE_INFO("INIT: after FD_SET UDP unicast bits=0x%08lx%08lx\n",
                    (unsigned long)(master_socket.bits >> 32),
                    (unsigned long)(master_socket.bits & 0xFFFFFFFFUL));

  OPENER_TRACE_INFO("INIT: before FD_SET UDP broadcast\n");
  FD_SET(g_network_status.udp_global_broadcast_listener, &master_socket);
  OPENER_TRACE_INFO("INIT: after FD_SET UDP broadcast bits=0x%08lx%08lx\n",
                    (unsigned long)(master_socket.bits >> 32),
                    (unsigned long)(master_socket.bits & 0xFFFFFFFFUL));

  OPENER_TRACE_INFO("INIT: before GetMaxSocket\n");
  highest_socket_handle = GetMaxSocket(g_network_status.tcp_listener,
                                       g_network_status.udp_global_broadcast_listener,
                                       0,
                                       g_network_status.udp_unicast_listener);
  OPENER_TRACE_INFO("INIT: after GetMaxSocket highest=%d\n", highest_socket_handle);

  OPENER_TRACE_INFO("INIT: before GetMilliSeconds\n");
  g_last_time = GetMilliSeconds(); /* initialize time keeping */
  OPENER_TRACE_INFO("INIT: after GetMilliSeconds=%lu\n", (unsigned long)g_last_time);
  g_network_status.elapsed_time = 0;
  OPENER_TRACE_INFO("INIT: NetworkHandlerInitialize complete\n");
...
void CheckAndHandleTcpListenerSocket(void)
{
  ...
  if (true == CheckSocketSet(g_network_status.tcp_listener))
  {
    ...
    {
      OPENER_TRACE_ERR("networkhandler: no free socket timer for TCP socket %d\n", new_socket);
      CloseSocket(new_socket);
      return;
    }

    /* The timer is associated with this socket when RegisterSession arrives. */
...
EipStatus NetworkHandlerProcessCyclic(void)
{
  ...
  int ready_socket = opener_select(highest_socket_handle + 1,
                                   &read_socket,
                                   0,
                                   0,
                                   &g_time_value);
...
void CheckAndHandleUdpGlobalBroadcastSocket(void)
{
  ...
  if (true == CheckSocketSet(g_network_status.udp_global_broadcast_listener))
  {
    ...
    static CipOctet incoming_message[PC_OPENER_ETHERNET_BUFFER_SIZE];
...
void CheckAndHandleConsumingUdpSocket(void)
{
  ...
  while (NULL != iterator)
  {
    ...
    if ((kEipInvalidSocket !=
        ...
        (true ==
         CheckSocketSet(current_connection_object->socket[kUdpCommuncationDirectionConsuming])))
    {
