    ...
    CipConnectionObject *const RESTRICT connection_object)
{
             ...
             ConnectionObjectGetTToOConnectionType(io_connection_object))
  {
        ...
        iterator->produced_path.instance_id)
    {
      ...
      {
        return kConnectionManagerExtendedStatusCodeMismatchedTToONetworkConnectionFixVar;
      }
      if (ConnectionObjectGetTToOPriority(io_connection_object) !=
          ConnectionObjectGetTToOPriority(iterator))
      {
        return kConnectionManagerExtendedStatusCodeMismatchedTToONetworkConnectionPriority;
      }
      ...
      if (ConnectionObjectGetTransportClassTriggerProductionTrigger(
              io_connection_object) != ConnectionObjectGetTransportClassTriggerProductionTrigger(iterator))
      {
      ...
      {
        return kConnectionManagerExtendedStatusCodeMismatchedTToOProductionInhibitTimeSegment;
      }
    }
    ...
    CipCommonPacketFormatData *const common_packet_format_data)
{
  ...
  struct sockaddr_in addr =
      {.sin_family = AF_INET, .sin_addr.s_addr = INADDR_ANY, .sin_port = htons(kOpenerEipIoUdpPort)};
...
EipStatus SendConnectedData(CipConnectionObject *connection_object)
{
  ...
  /*
   * Arduino Opta / Mbed modification:
   * Use static storage for the outgoing I/O message buffer rather than
   * allocating the ENIPMessage structure on the stack for every call.
   *
   * The message is still reinitialised on every call to SendConnectedData(),
   * so no packet data is intentionally retained between transmissions.
   *
   * was-
